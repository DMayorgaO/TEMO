...
DO $$ BEGIN
  CREATE TYPE tipo_arqueo AS ENUM ('APERTURA', 'CIERRE_CALCULADO', 'CIERRE_CONTADO', 'AJUSTE');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS arqueos (
  ...
  fecha_creacion TIMESTAMPTZ NOT NULL DEFAULT now(),
  observaciones TEXT,
  CONSTRAINT ck_arqueos_alcance CHECK (id_turno IS NOT NULL OR id_jornada IS NOT NULL)
);
...
VALUES
  ('DUENA', 'Duena', 'Acceso completo al sistema, comisiones y cierres.'),
...
SELECT r.id_rol, 'Propietaria', 'Olivera', 'duena', 'duena@temo.local', '$2b$10$pendiente', true
FROM roles r
WHERE r.codigo = 'DUENA'
