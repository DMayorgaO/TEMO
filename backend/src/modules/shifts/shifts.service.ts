...
export class ShiftsService {
  ...
  private async loadExpectedCash(shiftId: string) {
    const result = await this.db.query<{ currency: CurrencyCode; amount: string } & QueryResultRow>(
      `select
         m.codigo as currency,
         (
    ...
    return result.rows.reduce<Record<CurrencyCode, number>>(
      (acc, row) => ({ ...acc, [row.currency]: Number(row.amount) }),
      { NIO: 0, USD: 0 },
    );
  }
